...
- **Weapon contract**: every weapon is one file in `packages/core/tools/` registering `{name, icon, cursor, hit(ctx, x, y), sound()}`. Adding a weapon must not touch engine internals.
