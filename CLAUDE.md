...
- **Zero dependencies**: no bundler, no framework, no runtime deps (Electron + electron-builder are the only dev deps, desktop package only). All art is procedural canvas drawing; all audio is WebAudio synthesis. **Binary-asset exception**: app icons are PNGs, but they are *generated from procedural code* — no hand-drawn binary art. `npm run icons` (`scripts/make-icons.js`) rasterises them and encodes with Node's zlib; it is the source of truth. `scripts/make-icons.html` is a browser playground for the same design. Icons live in `packages/extension/icons/` and `packages/desktop/icon.png`. Never hand-transfer image bytes — a mangled base64 shipped a corrupt icon once and the Web Store rejected it.
